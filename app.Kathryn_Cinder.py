...
def calc_mos(audio_path, id, ref, pre_ppm):
    wav, sr = torchaudio.load(audio_path)
    print(wav.shape)
    # if wav.shape[0] == 2:
    wav = wav[0, :].unsqueeze(dim=0)
...
description ="""
This is the experiment page for Laronix Data Recording (For Laryngectomy Patient).\n
\n
...
""" 
## Auto load examples
refs = np.loadtxt("Cinder.txt", delimiter="\n", dtype="str")
refs_ids = [x.split(" ")[0] for x in refs]
refs_txt = [" ".join(x.split(" ")[1:]) for x in refs]
ref_feature = np.loadtxt("Patient_sil_trim_16k_normed_5_snr_40/Cinder.csv", delimiter=",", dtype="str")
ref_wavs =[str(x) for x in sorted(Path("Patient_sil_trim_16k_normed_5_snr_40/Cinder").glob("**/*.wav"))]
...
iface = gr.Interface(
    ...
    allow_flagging="manual",
    flagging_dir="Kathryn_Cinder",
    flagging_options=["Perfect","Doubtful_Speaking_Rate", "Douftful_Naturalness"],
    examples=examples,
...
demo.launch(share=True, auth=("Kathryn", "Laronix1124"))
