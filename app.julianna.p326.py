...
def calc_mos(audio_path, id, ref, pre_ppm, fig):
    ...
    if ppm >= float(pre_ppm) + 100:
        error_msg += "ERROR: Please speak slower.\n"
    elif ppm <= float(pre_ppm)- 100:
...
ref_wavs =[str(x) for x in sorted(Path("John_p326/output").glob("**/*.wav"))]
# ref_figs =[str(x) for x in sorted(Path("./Patient_sil_trim_16k_normed_5_snr_40/Arthur/ref_fig").glob("**/*.png"))]
...
demo.launch(share=True, auth=[("Julianna", "Laronix0929")])
