...
def calc_mos(audio_path, id, ref, pre_ppm):
    ...
    if ppm >= float(pre_ppm) + 300:
        error_msg += "ERROR: Please speak slower.\n"
    elif ppm <= float(pre_ppm)- 300:
...
Predicted MOS >=1.3 \n
Pef PPM - 300 < PPM < Ref PPM + 300 \n
""" 
## Auto load examples
refs = np.loadtxt("p326_split.txt", delimiter="\n", dtype="str")
refs_ids = [x.split(" ")[0] for x in refs]
refs_txt = [" ".join(x.split(" ")[1:]) for x in refs]
ref_feature = np.loadtxt("p326_split_slow_0.7_ref.csv", delimiter=",", dtype="str")
ref_wavs =[str(x) for x in sorted(Path("./p326_split_slow_0.7").glob("**/*.wav"))]
...
iface = gr.Interface(
    ...
    allow_flagging="manual",
    flagging_dir="Machael_p326_slow_0.7",
