...
""" 
## Auto load examples
refs = np.loadtxt("Julianna_Arthur_the_rat.txt", delimiter="\n", dtype="str")
refs_ids = [x.split(" ")[0] for x in refs]
refs_txt = [" ".join(x.split(" ")[1:]) for x in refs]
ref_feature = np.loadtxt("Julianna_pause_ref.csv", delimiter=",", dtype="str")
ref_wavs =[str(x) for x in sorted(Path("./Julianna_pause/output").glob("**/*.wav"))]
