...
# Path.mkdir(Path("ref_fig"), exist_ok=True)
# for i in ref_wavs:
#     y_p, sr_p = librosa.load(i, sr=16000)
# # y_h, sr_h = librosa.load("/home/kevingeng/laronix_automos/Julianna/output/Julianna_Set1_Author_01.wav", sr=48000)

# # a_h, p_h = get_speech_interval(y_h, db=40)
#     a_p, p_p = get_speech_interval(y_p, db=23)

#     fig_p = plot_UV(y_p, a_p, sr=sr_p)
#     id = Path(i).stem + ".png"
#     fig_p.savefig("ref_fig/%s"%id)
#     plt.clf()

# fig_h = plot_UV(y_h, a_h, sr=sr_h)
# pdb.set_trace()
# fig_p.savefig("p1.png")
# fig_h.savefig("h1.png")
...
ref_figs =[str(x) for x in sorted(Path("./Patient_sil_trim_16k_normed_5_snr_40/Arthur_the_rat/ref_fig").glob("**/*.png"))]
...
examples = [[w, i, x, y, z] for w, i, x, y, z in zip(ref_wavs, refs_ids, refs_txt, refs_ppm, ref_figs)]
try:
    examples != []
except:
    print("Examples loading error, Please check")
    exit()
...
iface = gr.Interface(
    ...
    allow_flagging="manual",
    flagging_dir="Mousa_Pause",
    flagging_options=["Perfect","Susbicious_Speaking_Rate", "Susbicious_Naturalness", "Susbicious_Pause"],
    examples=examples,
...
demo.launch(share=True, auth=[("Mousa", "Laronix1130")])
